...
| Envio | `senders/uazapi.ts` | `sendUazapiText` · `sendUazapiMedia` · `sendUazapiAudio` · `toUazapiNumber` |
...
| Rota webhook | `api/chat/webhook/uazapi/route.ts` | recebe eventos (secret, status, mídia, echo, inbound, lead, relay) |
...
| POST | `/send/text` | `{number, text, track_id, track_source}` | `{id, messageid, status}` |
| POST | `/send/media` | `{number, type, file, text?, docName?, track_id}` | `{id, messageid}` |
...
- **`EventType: "messages_update"`** → dados em **`payload.event`** (NÃO em `message`):
  ```
  { Type:"Delivered"|"Read"|"Played"|"Sent"|"Error"|"FileDownloaded",
