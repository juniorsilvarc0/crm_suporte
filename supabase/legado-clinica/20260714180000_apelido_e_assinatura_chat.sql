-- Apelido para atendimento + assinatura das mensagens do chat.
--
-- O operador pode assinar as mensagens que envia pelo CRM (o contato vê quem
-- está falando). A assinatura usa o apelido; se vazio, o primeiro nome. O
-- checkbox por usuário liga/desliga.

alter table public.app_users
  add column if not exists apelido_atendimento text,
  add column if not exists assinar_mensagens boolean not null default true;

-- ---------------------------------------------------------------------------
-- create_app_user: novos parâmetros COM DEFAULT — a chamada antiga (6 args)
-- continua resolvendo para esta função, sem janela de quebra no deploy.
-- ---------------------------------------------------------------------------
-- Dropa a assinatura anterior (6 args) E a nova (8 args), para a migration ser
-- re-executável com segurança — um `create or replace` com outra aridade criaria
-- uma sobrecarga ambígua em vez de substituir.
drop function if exists public.create_app_user(text, text, text, text, text, boolean);
drop function if exists public.create_app_user(text, text, text, text, text, boolean, text, boolean);
...
-- ---------------------------------------------------------------------------
-- update_app_user: novos parâmetros com DEFAULT NULL = "não altera".
--   apelido: null → mantém | '' → limpa | texto → define
--   assinar: null → mantém | true/false → define
-- Isso é essencial porque o toggle de ativar/desativar chama esta RPC SEM os
-- campos novos — sem o "não altera", desativar alguém apagaria o apelido dele.
--
-- ATENÇÃO: é preciso DROPAR a versão de 8 args antes de criar a de 10. Um
-- `create or replace` com outra aridade cria uma SOBRECARGA em vez de
-- substituir — e aí a chamada de 8 args vira ambígua ("function is not
-- unique"), quebrando desativar usuário, salvar perfil e avatar.
-- ---------------------------------------------------------------------------
drop function if exists public.update_app_user(
  uuid, uuid, text, text, boolean, text, text, text
);
drop function if exists public.update_app_user(
  uuid, uuid, text, text, boolean, text, text, text, text, boolean
);
